...
system_owner = os.environ['SystemOwner']
ALBAcessLogBucket = os.environ['ALBAcessLogBucket']
ALBName = os.environ['ALBName']
...
SecurityGroupId1 = os.environ['SecurityGroupId1']
SecurityGroupId2 = os.environ['SecurityGroupId2']
SslPolicy = os.environ['SslPolicy']
SubnetA = os.environ['SubnetA']
SubnetB = os.environ['SubnetB']
SubnetC = os.environ['SubnetC']
...
def main():
    ...
    template = parse_template("application-load-balancer-redirect.yml", credentials=credentials)
    ...
    cft_params = []
    cft_params.append({'Application': domain_name})
    cft_params.append({'SystemOwner': system_owner})
    ...
    cft_params.append({'SecurityGroupId1': SecurityGroupId1})
    cft_params.append({'SecurityGroupId2': SecurityGroupId2})
    cft_params.append({'SslPolicy': SslPolicy})
    cft_params.append({'SubnetA': SubnetA})
    cft_params.append({'SubnetB': SubnetB})
    cft_params.append({'SubnetC': SubnetC})
