...
def main():
    ...
    template = parse_template("../cloudformation/DOI-Standard-S3-template.yml", credentials=credentials)
    print ("Template validated!")

    stack_id = int(time())
    jenkins_job_name = os.environ['StackName']
    jenkins_build_number = os.environ['BUILD_NUMBER']
    stack_name = jenkins_job_name + "-" + "s3-cf"
    print ("Using stack name: " + stack_name)

    cft_params = []
    cft_params.append({'BucketName': BucketName})
    cft_params.append({'Versioning': Versioning})
    cft_params.append({'EnvironmentType': EnvironmentType})
    cft_params.append({'S3TagBusinessUnit': S3TagBusinessUnit})
    cft_params.append({'S3TagApplication': S3TagApplication})
    cft_params.append({'S3TagEnvironment': S3TagEnvironment})
    cft_params.append({'S3TagSystemOwner': S3TagSystemOwner})
    cft_params.append({'S3TagRFC': S3TagRFC})
    cft_params.append({'S3TagProjectCode': S3TagProjectCode})
    cft_params.append({'S3TagPublicAccess': S3TagPublicAccess})
